...
export class CurriculumEngine {
  ...
  public async validate(options: ValidationOptions = {}): Promise<ValidationSummary> {
    ...
    const tier4Dir = path.join(this.lessonsDir, 'tier4');
    ...
    const tier4Files = fs.existsSync(tier4Dir)
      ? fs.readdirSync(tier4Dir).filter((f) => f.startsWith('unit') && f.endsWith('.json'))
      : [];
    ...
    if (tierFilter === '1' || tierFilter === '2' || tierFilter === '3' || tierFilter === '4') {
    ...
    if (tierFilter === '2' || tierFilter === '3' || tierFilter === '4') {
    ...
    // 5. Process Tier 4 Files if requested
    if (tierFilter === '4' || tierFilter === 'all') {
      for (const file of tier4Files) {
        const filePath = path.join(tier4Dir, file);
        try {
          const content = JSON.parse(fs.readFileSync(filePath, 'utf8'));
          if (!content || typeof content !== 'object') {
            errors.push({
          ...
          // Stage 5: Interleaving Calculator (Units >= 2)
          const unitNumber = typeof content.unit_number === 'number' ? content.unit_number : 46;
          if (unitNumber >= 2 && cumulativeVocabLexicon.size > 0) {
            const stat = this.calculateInterleaving(content, cumulativeVocabLexicon);
            interleavingStats[unitId] = stat;
